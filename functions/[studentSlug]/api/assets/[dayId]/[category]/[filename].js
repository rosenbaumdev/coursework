// DELETE /<studentSlug>/api/assets/:dayId/:category/:filename — remove from R2.
...
export async function onRequestDelete({ params, env }) {
  const { studentSlug, dayId, category, filename } = params
  ...
  if (category === 'claude-prompt' && env.GITHUB_PAT) {
    await removeFromGitHub(course, dayId, filename, env.GITHUB_PAT)
  }
