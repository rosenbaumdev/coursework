...
| Workshop VM tunnel | The Day-2 live surface (terminal + app viewer) is served from the droplet (`146.190.131.15`, user `coder`) via a STABLE named Cloudflare tunnel at `workshop.kitbord.com` → droplet `proxy.cjs` (:9000, multiplexes app HTTP:8080 + PTY WS:7681). `--protocol http2` (never QUIC). `TERMINAL_WS_URL` + `APP_VIEWER_URL` (wrangler `[vars]`) point at it; the same URL is hardcoded as `DEFAULT_VM_URL` in `functions/_session.js` so a failed `[vars]` binding can't strand the viewer with an empty URL. `TERMINAL_TOKEN` is a secret (never defaulted, never committed). Do NOT go back to `*.trycloudflare.com` quick tunnels (they churn). See decisions.md "Workshop VM: stable named tunnel". |
